...
{
	/// <summary>
	/// Dmitry Bychenko's solution but with an inline matrix (based on DenseMatrix)
	/// </summary>
	public class DmitryInlineMatrix : ITextDiff
	{
			...
			int insertCost = 1, int removeCost = 1, int editCost = 1)
		{
			...
			var M = new EditOperationKind[columns * rows];

			// Minimum cost so far
			var D = new int[columns * rows];
